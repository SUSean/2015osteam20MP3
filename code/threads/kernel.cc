...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
        	...
        	execfile[++execfileNum]= argv[++i];
        	execfilePri[execfileNum]= argv[++i];
			cout << execfile[execfileNum] <<" "<<execfilePri[execfileNum] << "\n";
...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i],execfilePri[i]);
	}
