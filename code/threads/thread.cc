...
Thread::Yield ()
{
    ...
    kernel->scheduler->ReadyToRun(this);//can run same thread
    nextThread = kernel->scheduler->FindNextToRun();
    totalBurstTime+=kernel->stats->totalTicks-startTime;
    setStopTime(kernel->stats->totalTicks);
    if (nextThread != NULL) {
...
Thread::UpdateBurstTime(){
    ...
    burstCount++;
    if(burstCount==1)
        tempBurstTime=getTotalBurstTime();
    else if(burstCount==2)
        setBurstTime(0.5*getTotalBurstTime()+0.5*tempBurstTime);
    else
        setBurstTime(0.5*getTotalBurstTime()+0.5*lastBurstTime);
    lastBurstTime=getBurstTime();
    setTotalBurstTime(0);
}
